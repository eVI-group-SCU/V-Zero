...
The complete training implementation lives in the bundled `verl` source tree:
...
TEACHER_MODEL=/path/to/Qwen3.5-27B \
TRAIN_FILE=/path/to/train.parquet \
VAL_FILE=/path/to/val.parquet \
RUN_TAG=vzero_qwen35_27b_4b \
bash verl/examples/on_policy_distillation_trainer/run_vzero_qwen35_vl_fsdp.sh
```
...
```bash
ROLLOUT_N=12 bash verl/examples/on_policy_distillation_trainer/run_vzero_qwen35_vl_fsdp.sh
```

Reduce vLLM rollout memory reservation:

```bash
ROLLOUT_GPU_MEM_UTIL=0.35 TEACHER_GPU_MEM_UTIL=0.45 bash verl/examples/on_policy_distillation_trainer/run_vzero_qwen35_vl_fsdp.sh
```
