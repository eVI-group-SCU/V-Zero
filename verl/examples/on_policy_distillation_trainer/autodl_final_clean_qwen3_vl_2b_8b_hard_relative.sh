...
STUDENT_MODEL=${STUDENT_MODEL:-${MODEL_ROOT}/Qwen3-VL-2B-Instruct}
...
if [[ "${teacher_ema_enabled}" == "True" || "${teacher_ema_enabled}" == "true" || "${teacher_ema_enabled}" == "1" ]]; then
    ...
    TEACHER_MODEL=${TEACHER_MODEL:-${MODEL_ROOT}/Qwen3-VL-8B-Instruct}
fi
...
run_tag=${RUN_TAG:-autodl_qwen3_vl_2b_student_8b_hard_relative}
rollout_data_dir=${ROLLOUT_DATA_DIR:-./final_logs_hard_relative/${run_tag}/rollouts}
validation_data_dir=${VALIDATION_DATA_DIR:-./final_logs_hard_relative/${run_tag}/val_generations}
...
filtered_train_file=${FILTERED_TRAIN_FILE:-/root/autodl-tmp/dataset/ZwZ-RL-VQA/verl_format_teacher_crop_train37k_filtered_maxprompt25000_teachermax32768_hardneg_with_bbox_length10_512/train.parquet}
