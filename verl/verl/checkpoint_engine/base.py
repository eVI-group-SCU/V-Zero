...
class CheckpointEngineManager:
    ...
    @auto_await
    async def update_weights(self, global_steps: int = None, weight_source: str = "student"):
        """Update weights from trainer to rollout replicas.

        Args:
            global_steps: The global steps of the trainer.
            weight_source: Which actor-side weight source to export. ``"student"`` keeps the existing behavior;
                ``"ema"`` exports the CPU EMA teacher state.
        """

        # 0. update weights for sync training with colocated trainer and rollout
        if self.backend == "naive":
            ray.get(
                self.trainer.update_weights(
        ...
        ray.get(
            trainer.update_weights(global_steps=global_steps, mode=self.backend, weight_source=weight_source)
