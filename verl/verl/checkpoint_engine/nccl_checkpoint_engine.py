...
class BroadcastOperation:
    """Async broadcast operation with metadata handshake and NCCL in a separate thread.
    ...
    Args:
        ...
        topic (str): The metadata channel name used in the request handshake.
        num_metadata_receivers (int): Number of rollout ranks that should request metadata from rank 0.
    """
    ...
    def __init__(
        ...
        self.socket = socket
        self.topic = topic
        self.num_metadata_receivers = num_metadata_receivers

        loop = asyncio.get_running_loop()
        self._task = loop.run_in_executor(None, self._run)

    def _run(self):
        # Exchange tensor metadata via ZeroMQ REQ/REP before the NCCL broadcast.
        #
        # This used to be PUB/SUB, which can drop the first message while
        # subscriptions are still propagating. If that happens, rank 0 enters
        # NCCL broadcast while rollout ranks wait forever for metadata.
        if self.rank == 0:
            for _ in range(self.num_metadata_receivers):
                topic = self.socket.recv_string()
                if topic != self.topic:
                    raise RuntimeError(f"Unexpected metadata topic {topic!r}, expected {self.topic!r}")
                self.socket.send_pyobj(self.metadata)
        else:
            self.socket.send_string(self.topic)
...
class NCCLCheckpointEngine(CheckpointEngine):
    ...
    def __init__(
        ...
        # Start ZeroMQ server for bucket tensor metadata handshakes.
        ...
        async for tensor_meta, chunk in split_weight_chunks(weights, self.bucket_size):
            ...
            if offset + tensor_meta.chunk_size > self.bucket_size:
                ...
                broadcast_op = BroadcastOperation(
                    ...
                    socket=self.socket,
                    topic=self.topic,
                    num_metadata_receivers=self.world_size - 1,
                )
