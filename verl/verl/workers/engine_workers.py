...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_ema_state(self, dtype: str = "bf16", device: str = "cpu", init_from_student: bool = True):
        assert "actor" in self.role, "init_ema_state only supports actor role"
        return self.actor.engine.init_ema_state(dtype=dtype, device=device, init_from_student=init_from_student)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_ema_state(self, decay: float, global_step: int = None):
        assert "actor" in self.role, "update_ema_state only supports actor role"
        return self.actor.engine.update_ema_state(decay=decay, global_step=global_step)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_ema_state(self, local_path: str):
        assert "actor" in self.role, "load_ema_state only supports actor role"
        return self.actor.engine.load_ema_state(local_path=local_path)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL, blocking=False)
    async def update_weights(self, global_steps: int = None, mode: str = "auto", weight_source: str = "student"):
        ...
        Args:
            ...
            weight_source: ``"student"`` exports the current actor parameters. ``"ema"`` exports the actor-side
                EMA state for teacher rollout synchronization.
        """

        if weight_source not in {"student", "ema"}:
        ...
        if effective_mode != "naive":
            ...
            try:
                # Some distributed model engines, especially FSDP2/DTensor, need every
                # trainer rank to participate while materializing full tensors.  The
                # checkpoint engine decides which trainer rank actually sends data;
                # non-sender ranks still consume this iterator to drive collectives.
                per_tensor_param, _ = self.actor.engine.get_per_tensor_param(weight_source=weight_source)
        ...
        if weight_source != "student":
            raise ValueError("weight_source='ema' is not supported with naive checkpoint_engine backend.")
        ...
        per_tensor_param, peft_config = self.actor.engine.get_per_tensor_param(
            layered_summon=self.layered_summon, base_sync_done=True, weight_source=weight_source
        )
        ...
        if do_lora_base_sync:
            per_tensor_param_base, peft_config = self.actor.engine.get_per_tensor_param(
                layered_summon=self.layered_summon, base_sync_done=False, weight_source=weight_source
            )
