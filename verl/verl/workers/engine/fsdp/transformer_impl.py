...
class FSDPEngine(BaseEngine):
    ...
    def __init__(
        ...
        self._ema_dtype: torch.dtype | None = None
        self._ema_device = torch.device("cpu")
        self._ema_requested_device = "cpu"
        self._ema_update_count = 0
        self._ema_last_global_step: int | None = None
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        if self._ema_state is not None:
            self.save_ema_state(local_path=local_path, global_step=global_step)
    ...
    @staticmethod
    def _resolve_ema_dtype(dtype: str | torch.dtype) -> torch.dtype:
        ...
        raise ValueError(f"Unsupported EMA dtype {dtype!r}; expected bf16/bfloat16 or fp32/float32.")

    def _current_accelerator_device(self) -> torch.device:
        if device_name == "cpu":
            return torch.device("cpu")
        return torch.device(device_name, get_device_id())

    def _resolve_ema_device(self, device: str | torch.device) -> torch.device:
        if isinstance(device, torch.device):
            if device.type == "cpu":
                return device
            if device.index is None:
                return torch.device(device.type, get_device_id())
            return device

        normalized = str(device).lower()
        if normalized == "cpu":
            return torch.device("cpu")
        if normalized not in {"gpu", "cuda", "npu"}:
            raise ValueError(f"Unsupported EMA device {device!r}; expected cpu, gpu, cuda, or npu.")
        if device_name == "cpu":
            raise ValueError("Teacher EMA device was set to GPU, but no accelerator device is available.")
        if normalized in {"cuda", "npu"} and normalized != device_name:
            raise ValueError(
                f"Teacher EMA device {normalized!r} does not match the current accelerator {device_name!r}."
            )
        return self._current_accelerator_device()

    def _ema_on_accelerator(self) -> bool:
        return self._ema_device.type != "cpu"

    @staticmethod
    def _update_ema_tensor_(ema_tensor: torch.Tensor, current_tensor: torch.Tensor, decay: float) -> None:
        if decay == 0 or not ema_tensor.is_floating_point():
            ema_tensor.copy_(current_tensor)
            return
        ema_tensor.mul_(decay).add_(current_tensor, alpha=1 - decay)

    def _check_ema_supported(self) -> None:
        if self._is_lora:
            raise NotImplementedError("Teacher EMA first pass does not support LoRA models.")
        if self._qat_enabled:
            raise NotImplementedError("Teacher EMA first pass does not support QAT models.")

    def _collect_current_params_for_ema(self) -> dict[str, torch.Tensor]:
        log_gpu_memory_usage("Before EMA load_fsdp_model_to_gpu", logger=logger)
        load_fsdp_model_to_gpu(self.module)
        log_gpu_memory_usage("After EMA load_fsdp_model_to_gpu", logger=logger)

        params = self.module.state_dict()
        params = convert_weight_keys(params, getattr(self.module, "_fsdp_wrapped_module", self.module))

        log_gpu_memory_usage("Before EMA offload_fsdp_model_to_cpu", logger=logger)
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.module)
        log_gpu_memory_usage("After EMA offload_fsdp_model_to_cpu", logger=logger)
        return params

    def _param_to_ema_storage(self, param) -> torch.Tensor:
        if isinstance(param, DTensor):
            local_param = param.to_local().detach()
            if local_param.is_floating_point():
                return local_param.to(device=self._ema_device, dtype=self._ema_dtype).clone()
            return local_param.to(device=self._ema_device).clone()
        if not isinstance(param, torch.Tensor):
            raise NotImplementedError(
                f"Teacher EMA only supports Tensor/DTensor state_dict values, got {type(param).__name__}."
            )
        param = param.detach()
        if param.is_floating_point():
            return param.to(device=self._ema_device, dtype=self._ema_dtype).clone()
        return param.to(device=self._ema_device).clone()

    def init_ema_state(self, dtype: str = "bf16", device: str = "cpu", init_from_student: bool = True) -> dict:
        self._check_ema_supported()

        self._ema_dtype = self._resolve_ema_dtype(dtype)
        self._ema_device = self._resolve_ema_device(device)
        self._ema_requested_device = str(device).lower()
        self._ema_update_count = 0
        self._ema_last_global_step = None
        self._ema_state = {}

        start = time.time()
        num_tensors = 0
        numel = 0
        if init_from_student:
            with torch.no_grad():
                params = self._collect_current_params_for_ema()
                try:
                    for name, param in params.items():
                        ema_tensor = self._param_to_ema_storage(param)
                        self._ema_state[name] = ema_tensor
                        num_tensors += 1
                        numel += ema_tensor.numel()
                finally:
                    del params
                    gc.collect()

        return {
            "teacher_ema/init_seconds": time.time() - start,
            "teacher_ema/num_tensors": num_tensors,
            "teacher_ema/numel": numel,
            "teacher_ema/storage_on_accelerator": int(self._ema_on_accelerator()),
        }

    def update_ema_state(self, decay: float, global_step: int = None) -> dict:
        self._check_ema_supported()
        if self._ema_state is None:
            raise RuntimeError("Teacher EMA state is not initialized. Call init_ema_state() first.")
        if not 0 <= decay < 1:
        ...
        with torch.no_grad():
            ...
            try:
                for name, param in params.items():
                    current = self._param_to_ema_storage(param)
                    if name not in self._ema_state:
                        self._ema_state[name] = current
                    else:
                        ema_tensor = self._ema_state[name]
                        if ema_tensor.shape != current.shape:
                            raise ValueError(
                                f"EMA tensor shape mismatch for {name}: stored {tuple(ema_tensor.shape)} vs "
                                f"current {tuple(current.shape)}."
                            )
                        self._update_ema_tensor_(ema_tensor, current, decay=decay)
                    updated += 1
                    numel += current.numel()
                    del current
            finally:
        ...
        return {
            "teacher_ema/update_seconds": time.time() - start,
            "teacher_ema/update_count": self._ema_update_count,
            "teacher_ema/updated_tensors": updated,
            "teacher_ema/numel": numel,
            "teacher_ema/decay": decay,
            "teacher_ema/storage_on_accelerator": int(self._ema_on_accelerator()),
        }

    def save_ema_state(self, local_path: str, global_step: int = 0) -> dict:
        if self._ema_state is None:
            return {"teacher_ema/saved": False}

        ema_dir = os.path.join(local_path, "ema_state")
        os.makedirs(ema_dir, exist_ok=True)
        rank_path = os.path.join(ema_dir, f"rank_{self.rank}.pt")
        torch.save(
            {
                "state": {name: tensor.detach().cpu() for name, tensor in self._ema_state.items()},
                "dtype": str(self._ema_dtype).replace("torch.", ""),
                "device": self._ema_requested_device,
                "global_step": global_step,
                "update_count": self._ema_update_count,
                "last_global_step": self._ema_last_global_step,
            },
            rank_path,
        )
        if self.rank == 0:
            metadata_path = os.path.join(ema_dir, "metadata.json")
        ...
        return {"teacher_ema/saved": True}

    def load_ema_state(self, local_path: str) -> bool:
        ema_dir = os.path.join(local_path, "ema_state")
        rank_path = os.path.join(ema_dir, f"rank_{self.rank}.pt")
        ...
        if not isinstance(state, dict):
            raise ValueError(f"Invalid EMA checkpoint payload at {rank_path}: missing state dict.")

        payload_dtype = payload.get("dtype")
        if payload_dtype is not None:
            self._ema_dtype = self._resolve_ema_dtype(payload_dtype)
        elif self._ema_dtype is None:
            self._ema_dtype = torch.bfloat16

        self._ema_state = {}
        for name, tensor in state.items():
            self._ema_state[name] = self._param_to_ema_storage(tensor)
        self._ema_update_count = int(payload.get("update_count", 0))
        self._ema_last_global_step = payload.get("last_global_step", payload.get("global_step"))
        return True

    def _ema_tensor_for_export(self, name: str, current_param):
        if self._ema_state is None:
            raise RuntimeError("Teacher EMA state is not initialized. Call init_ema_state() first.")
        if name not in self._ema_state:
            raise KeyError(f"EMA state does not contain parameter {name!r}.")

        ema_tensor = self._ema_state[name]
        device = self._current_accelerator_device()
        if isinstance(current_param, DTensor):
            local_tensor = ema_tensor.to(device, non_blocking=True)
            dtensor = DTensor.from_local(
                local_tensor,
                device_mesh=current_param.device_mesh,
                placements=current_param.placements,
                shape=current_param.shape,
                stride=current_param.stride(),
            )
            full_tensor = dtensor.full_tensor()
            if full_tensor.is_floating_point():
                return full_tensor.to(torch.bfloat16, non_blocking=True)
            return full_tensor
        return ema_tensor.to(device, non_blocking=True)

    def _get_ema_per_tensor_param(self):
        params = self._collect_current_params_for_ema()

        def iter_ema_params(params):
            try:
                for name, param in params.items():
                    tensor = self._ema_tensor_for_export(name, param)
                    try:
                        yield name, tensor
                    finally:
                        del tensor
        ...
        per_tensor_param = iter_ema_params(params)
    ...
    def get_per_tensor_param(
        ...
        **kwargs,
        ...
        if weight_source == "ema":
            return self._get_ema_per_tensor_param()
        if weight_source != "student":
            raise ValueError(f"Unsupported weight_source={weight_source!r}; expected 'student' or 'ema'.")
