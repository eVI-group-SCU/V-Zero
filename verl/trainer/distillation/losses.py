...
def compute_distillation_abs_loss(distillation_losses: torch.Tensor, response_mask: torch.Tensor) -> Metric:
    """Compute mean absolute distillation signal over valid response tokens."""
    if response_mask.is_nested:
        distillation_losses_response = distillation_losses[response_mask.bool().to_padded_tensor(False)]
    else:
        distillation_losses_response = distillation_losses[response_mask.bool()]
    return Metric(AggregationType.MEAN, distillation_losses_response.abs().mean())
...
def distillation_loss(
    ...
    if "distillation/abs_loss" in distillation_metrics:
        distillation_metrics["distillation/raw_abs_loss"] = distillation_metrics["distillation/abs_loss"]
    ...
    opd_weights = data.get("opd_weights", None)
    if opd_weights is not None:
        if opd_weights.is_nested:
            opd_weights = opd_weights.to_padded_tensor(1.0)
        opd_weights = opd_weights.to(device=distillation_losses.device, dtype=distillation_losses.dtype).detach()
        if opd_weights.dim() == 1:
            opd_weights = opd_weights.unsqueeze(-1)
        distillation_losses = distillation_losses * opd_weights

    if "distillation/abs_loss" in distillation_metrics:
        distillation_metrics["distillation/abs_loss"] = compute_distillation_abs_loss(
