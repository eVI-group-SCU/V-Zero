...
class TaskRunner:
    ...
    def run(self, config):
        ...
        maybe_print_first_train_sample(config.data, train_dataset)
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, is_train=True, max_samples: int = -1):
    ...
    teacher_crop_config = OmegaConf.select(data_config, "teacher_crop") or {}
    if not is_train and teacher_crop_config.get("enabled", False):
        teacher_crop_config = OmegaConf.merge(teacher_crop_config, {"enabled": False})
    effective_data_config = OmegaConf.merge(data_config, {"teacher_crop": teacher_crop_config})

    # Get the dataset class
    dataset_cls = get_dataset_class(effective_data_config)
...
def _safe_sample_value(value):
    ...
    if isinstance(value, Mapping):
        return {str(key): _safe_sample_value(val) for key, val in value.items()}
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        return [_safe_sample_value(item) for item in value]
    ...
    if size is not None and mode is not None:
        return f"<{type(value).__name__} size={size} mode={mode}>"
...
def _has_system_message(messages) -> bool:
    if not isinstance(messages, Sequence) or isinstance(messages, (str, bytes, bytearray)):
        return False
    return any(isinstance(message, Mapping) and message.get("role") == "system" for message in messages)


def maybe_print_first_train_sample(data_config, train_dataset) -> None:
    if not data_config.get("print_first_train_sample", False):
        return
    if len(train_dataset) == 0:
        print("==== first train sample ====")
        print("train dataset is empty")
        print("==== end first train sample ====")
        return

    sample = train_dataset[0]
    teacher_crop_config = OmegaConf.select(data_config, "teacher_crop") or {}
    teacher_crop_prompt_key = teacher_crop_config.get("prompt_key", "teacher_raw_prompt")
    keys = ["data_source", "raw_prompt", teacher_crop_prompt_key, "image_path", "reward_model", "extra_info"]
    payload = {key: _safe_sample_value(sample[key]) for key in keys if key in sample}
    payload["has_system_message"] = _has_system_message(sample.get("raw_prompt"))
    if teacher_crop_prompt_key in sample:
        payload[f"{teacher_crop_prompt_key}_has_system_message"] = _has_system_message(sample[teacher_crop_prompt_key])
    payload["data_use_system_prompt"] = _safe_sample_value(data_config.get("use_system_prompt", True))
