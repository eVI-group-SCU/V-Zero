...
class ServerAdapter(BaseRollout):
    ...
    def __init__(
        ...
        replica_rank: int = -1,
        weight_transfer_namespace: Optional[str] = None,
        server_role: str = "rollout",
        server_name_suffix: str = "",
    ):
        super().__init__(config, model_config, device_mesh)
        self.server_handle: ray.actor.ActorHandle = None
        self.server_role = server_role
        self.server_name_suffix = server_name_suffix
        ...
        namespace = weight_transfer_namespace or os.environ.get("VERL_WEIGHT_TRANSFER_NAMESPACE", "rollout")
        self.zmq_handle = (
            f"ipc:///tmp/rl-colocate-zmq-{job_id}-{namespace}-replica-{self.replica_rank}-rank-{local_rank}.sock"
        )
    ...
    async def update_weights(
        ...
        # Reset rollout caches after updating weights. For VL models, multi-modal
        # encoder caches depend on model weights and must not survive a sync.
        if self.rollout_rank == 0:
    ...
    def _get_server_actor_name(self) -> str:
        ...
        if self.server_role == "reward":
            return f"{prefix}server_reward_{self.replica_rank}_{self.node_rank}{self.server_name_suffix}"
        if self.server_role == "teacher":
            return f"{prefix}server_teacher_{self.replica_rank}_{self.node_rank}{self.server_name_suffix}"
        return f"{prefix}server_{self.replica_rank}_{self.node_rank}{self.server_name_suffix}"
