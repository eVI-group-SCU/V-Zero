...
class BaseEngine:
    ...
    def init_ema_state(self, dtype: str = "bf16", device: str = "cpu", init_from_student: bool = True) -> dict:
        """Initialize actor-side EMA state if the engine supports teacher EMA."""
        raise NotImplementedError(f"{type(self).__name__} does not support teacher EMA state.")

    def update_ema_state(self, decay: float, global_step: int = None) -> dict:
        """Update actor-side EMA state if the engine supports teacher EMA."""
        raise NotImplementedError(f"{type(self).__name__} does not support teacher EMA state.")

    def load_ema_state(self, local_path: str) -> bool:
        """Load actor-side EMA state from an actor checkpoint path if supported."""
        raise NotImplementedError(f"{type(self).__name__} does not support teacher EMA state.")
