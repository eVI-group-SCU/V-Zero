...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.teacher_crop_config = config.get("teacher_crop", {}) or {}
        ...
        self.teacher_crop_negative_image_key = self.teacher_crop_config.get("negative_image_key", None)
        self.teacher_crop_negative_prompt_key = self.teacher_crop_config.get(
        ...
        self.use_system_prompt = config.get("use_system_prompt", True)
    ...
    def _build_messages(
        ...
        images = example.get(image_key or self.image_key, None) or []
        crop_images = (example.get(crop_image_key, None) or []) if crop_image_key is not None else []
        videos = example.get(self.video_key, None) or []
        audios = example.get(self.audio_key, None) or []

        image_offset, crop_image_offset, video_offset, audio_offset = 0, 0, 0, 0
        for message in messages:
            if not images and not crop_images and not videos and not audios:
                continue
            ...
            if isinstance(content, list):
                for part in content:
                    if isinstance(part, dict) and part.get("type") == "image":
                        self._add_image_pixel_bounds(part)
                continue
            ...
            if crop_image_key is not None:
                placeholders = "<image>|<crop>|<video>|<audio>"
            segments = re.split(f"({placeholders})", content)
            segments = [item for item in segments if item != ""]
            for segment in segments:
                if segment == "<image>":
                    assert image_offset < len(images), f"image_offset {image_offset} >= len(images) {len(images)}"
                    content_list.append(self._build_image_content(images[image_offset], field_name="image"))
                    image_offset += 1
                elif segment == "<crop>":
                    assert crop_image_offset < len(crop_images), (
                        f"crop_image_offset {crop_image_offset} >= len(crop_images) {len(crop_images)}"
                    )
                    content_list.append(self._build_image_content(crop_images[crop_image_offset], field_name="crop"))
                    crop_image_offset += 1
                elif segment == "<video>":
        ...
        if self.use_system_prompt and system_prompt and not has_system_message:
            messages.insert(0, {"role": "system", "content": str(system_prompt)})
        return messages

    def _add_image_pixel_bounds(self, content: dict) -> None:
        min_pixels = getattr(self, "image_min_pixels", None)
        max_pixels = getattr(self, "image_max_pixels", None)
        if min_pixels is not None and "min_pixels" not in content:
            content["min_pixels"] = int(min_pixels)
        if max_pixels is not None and "max_pixels" not in content:
            content["max_pixels"] = int(max_pixels)

    def _build_image_content(self, image: Any, *, field_name: str) -> dict:
        if isinstance(image, Image.Image):
            content = {"type": "image", "image": image if image.mode == "RGB" else image.convert("RGB")}
        elif isinstance(image, dict):
            image_payload = dict(image)
            if "bytes" in image_payload:
                image_payload["image"] = Image.open(BytesIO(image_payload["bytes"]))
            content = {"type": "image", **image_payload}
        else:
            raise TypeError(f"{field_name} image must be dict or PIL.Image, unsupported image type: {type(image)}")
        self._add_image_pixel_bounds(content)
        return content

    @staticmethod
    def _extract_image_path(image: Any) -> str | None:
        if isinstance(image, str):
            return image
        if isinstance(image, dict):
            for key in ("image", "image_path", "path", "url", "image_url"):
                value = image.get(key)
    ...
    @staticmethod
    def _short_image_path(path: str | None) -> str | None:
        if path is None:
            return path
        return path.rstrip("/\\").replace("\\", "/").rsplit("/", 1)[-1]

    def _extract_image_paths(self, row_dict: dict) -> str | list[str] | None:
        images = row_dict.get(self.image_key, None)
        if images is None:
            return None
        if not isinstance(images, (list, tuple, ListConfig)):
            return self._short_image_path(self._extract_image_path(images))

        paths = [
            stripped_path
            for image in images
            if (path := self._extract_image_path(image)) is not None
            if (stripped_path := self._short_image_path(path)) is not None
        ]
        if len(paths) == 1:
            return paths[0]
    ...
    def __getitem__(self, item):
        ...
        row_dict["image_path"] = self._extract_image_paths(row_dict)
        row_dict["raw_prompt"] = self._build_messages(row_dict, key=self.prompt_key)
        if self.teacher_crop_enabled:
            if self.teacher_crop_prompt_key not in row_dict or row_dict[self.teacher_crop_prompt_key] is None:
                raise ValueError(
                    f"teacher_crop.enabled=True but row is missing {self.teacher_crop_prompt_key!r}"
                )
            if self.teacher_crop_image_key not in row_dict or row_dict[self.teacher_crop_image_key] is None:
                raise ValueError(f"teacher_crop.enabled=True but row is missing {self.teacher_crop_image_key!r}")
            teacher_images = row_dict[self.teacher_crop_image_key]
            if hasattr(teacher_images, "__len__") and len(teacher_images) == 0:
                raise ValueError(f"teacher_crop.enabled=True but {self.teacher_crop_image_key!r} is empty")
            raw_teacher_prompt = row_dict[self.teacher_crop_prompt_key]
            row_dict[self.teacher_crop_prompt_key] = self._build_messages(
                row_dict,
                key=self.teacher_crop_prompt_key,
                crop_image_key=self.teacher_crop_image_key,
            )
            negative_image_key = getattr(self, "teacher_crop_negative_image_key", None)
            if negative_image_key:
                if negative_image_key not in row_dict or row_dict[negative_image_key] is None:
                    raise ValueError(f"teacher_crop.enabled=True but row is missing {negative_image_key!r}")
                negative_images = row_dict[negative_image_key]
                if hasattr(negative_images, "__len__") and len(negative_images) == 0:
                    raise ValueError(f"teacher_crop.enabled=True but {negative_image_key!r} is empty")
                negative_example = dict(row_dict)
                negative_example[self.teacher_crop_prompt_key] = raw_teacher_prompt
                negative_prompt_key = getattr(self, "teacher_crop_negative_prompt_key", "teacher_neg_raw_prompt")
                row_dict[negative_prompt_key] = self._build_messages(
                    negative_example,
                    key=self.teacher_crop_prompt_key,
                    crop_image_key=negative_image_key,
                )
        ...
        if self.teacher_crop_enabled:
            row_dict.pop(self.teacher_crop_image_key, None)
            negative_image_key = getattr(self, "teacher_crop_negative_image_key", None)
            if negative_image_key:
                row_dict.pop(negative_image_key, None)
