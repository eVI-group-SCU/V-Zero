...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        with simple_timer("preprocess_multi_modal_info", metrics):
        ...
        while state != AgentState.TERMINATED:
            if state == AgentState.PENDING:
                with simple_timer("state_pending", metrics):
                    state = await self._handle_pending_state(agent_data, sampling_params)
            elif state == AgentState.GENERATING:
                with simple_timer("state_generating", metrics):
                    state = await self._handle_generating_state(agent_data, sampling_params)
            elif state == AgentState.PROCESSING_TOOLS:
                with simple_timer("state_processing_tools", metrics):
        ...
        # Finalize output
        with simple_timer("build_output", metrics):
    ...
    async def _handle_pending_state(self, agent_data: AgentData, sampling_params: dict[str, Any]) -> AgentState:
        ...
        with simple_timer("apply_chat_template", agent_data.metrics):
    ...
    async def _handle_generating_state(
        ...
        with simple_timer("tool_parse", agent_data.metrics):
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        with simple_timer("tool_build_messages", agent_data.metrics):
        ...
        with simple_timer("tool_response_tokenize", agent_data.metrics):
        ...
        with simple_timer("tool_update_state", agent_data.metrics):
    ...
    async def _call_tool(
        ...
        # Validate tool name
        with simple_timer("tool_call_validate", agent_data.metrics):
        ...
        try:
            ...
            if tool and instance_id and not isinstance(tool, FunctionTool):
                with simple_timer("tool_call_release", agent_data.metrics):
                    await tool.release(instance_id)

        with simple_timer("tool_response_format", agent_data.metrics):
