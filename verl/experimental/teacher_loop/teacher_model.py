...
class TeacherModelManager:
    ...
    def __init__(
        ...
        checkpoint_engine_config: Optional[CheckpointEngineConfig] = None,
    ):
        """
        ...
        Args:
            ...
            checkpoint_engine_config (Optional[CheckpointEngineConfig]): Checkpoint engine override for teacher
                rollout workers.
        """
        ...
        self.checkpoint_engine_config = checkpoint_engine_config
    ...
    def _initialize_llm_servers(self):
        ...
        if self.checkpoint_engine_config is not None:
            rollout_config = replace(rollout_config, checkpoint_engine=self.checkpoint_engine_config)
    ...
    def get_replicas(self):
        """Return rollout replicas backing this teacher model."""
...
class MultiTeacherModelManager:
    ...
    def _initialize_teacher_model_managers(self):
        ...
        for (key, teacher_model_config), teacher_pool in zip(teacher_models.items(), split_pools, strict=True):
            manager = TeacherModelManager(
                ...
                checkpoint_engine_config=self.checkpoint_engine_config,
            )
    ...
    def get_replicas(self):
        """Return rollout replicas for the single-teacher first-pass EMA path."""
        if len(self.teacher_model_managers) != 1:
            raise ValueError("Teacher EMA first pass requires exactly one teacher model.")
        manager = next(iter(self.teacher_model_managers.values()))
        return manager.get_replicas()
